...
// We can't use `rand::Rng::shuffle` because it uses `gen_range::<usize>` which concatenates two
// `next_u32`s on 64-bit machines.  If an exact match is not required it's fine.
...
pub fn deal_rand_board() -> Vec<String> {
    let mut deck = gen_deck();
    rand::weak_rng().shuffle(&mut deck);
    deck.chunks(8).map(|row| row.join(" ")).collect::<Vec<_>>()
}
